		...
		return this.each(function(){
			...
			if ($('#'+id).length == 0) {
				...
				table.append(
					$(options.rowTpl)
						...
						.append($(options.cellTpl).append($(options.buttonFunctionTpl).html(options.textDelete).addClass('del').click(function(){
							...
							if (nmpd.getValue().indexOf("%")<0)
								percentageInserted = false;
					...
					$(options.rowTpl)
						...
						.append($(options.cellTpl).append($(options.buttonFunctionTpl).html(options.percentage).addClass('sep').click(function(){
							if (nmpd.getValue().indexOf("%")<0 && nmpd.getValue().trim()!="") {
								nmpd.setValue(nmpd.getValue().toString() + options.percentage);
								percentageInserted = true;

							}
						})))
						.append($(options.zeroCellTpl).append($(options.buttonNumberTpl).html(0).addClass('numero')))
				...
				$('#'+id+' .numero').bind('click', function(){
					if(percentageInserted) return;
					var val;
				//	alert($(this).text()+"   "+$('#'+id+' .dirty').val()+"   "+id)
					if ($('#'+id+' .dirty').val() == '0'){
						val = $(this).text();
					} else {
			...
			*/
			nmpd.getValue = function(){
				return isNaN(nmpd.display.val() && nmpd.display.val().indexOf("%")<0) ? 0 : nmpd.display.val();
			};
